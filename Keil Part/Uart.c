...
void UART0_Init(uint32_t baud_rate)
{
	...
	UART0->S2&=~UART0_S2_MSBF_MASK;//and cu 0 sa fie setat  => lsb first
	
	//UART0->C3|=UART0_C3_TXINV_MASK; // tx inverted
	...
	uint16_t sbr = (uint16_t)((DEFAULT_SYSTEM_CLOCK)/(baud_rate * (osr+12))); // !!!
