...
void ADC0_Init() {
	...
	//conversii single-ended precizie 10
	ADC0->SC1[0]&=~ADC_SC1_DIFF_MASK; //diff=0
	ADC0->CFG1|=ADC_CFG1_MODE(10); //pag466 mode=10
...
void ADC0_IRQHandler(){
	...
	// Dezactivarea intreruperii de ADC
   ADC0->SC1[0] &= ~ADC_SC1_AIEN_MASK;

	//if((ADC0->SC1[0] & ADC_SC1_COCO_MASK) && ((ADC0->SC1[0] & ADC_SC1_ADCH_MASK) == LIGHT_CHANNEL)) {

	uint16_t analog_input = (uint16_t) ADC0->R[0];

	float measured_voltage =(analog_input*10.0f)/ 65535;
 
	
	if(measured_voltage>=stg && measured_voltage<=stop1) //interval 1 
	{
			unghi=350;  // 0
	}
	else if(measured_voltage>stop1 && measured_voltage<=stop2) // interval 2
	{
			unghi=200; // 90
	}
	else if(measured_voltage>stop2 && measured_voltage<=dr) //interval 3
	{
